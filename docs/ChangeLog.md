...
- 放宽 `user_version > 8` 的硬拒绝逻辑：当数据库版本较新但当前版本所需表结构完整时，允许以兼容模式继续启动。
- 启动兼容检查前会先执行一轮非破坏性补表补列，补齐当前代码仍然依赖、但旧 `create_tables` 不会自动覆盖的字段。
- 对真正不兼容的高版本数据库，错误信息现在会直接指出缺失的表/列，避免只看到“版本过新”却不知道差在哪。

### 数据库回归测试

- 新增“高版本但结构兼容可通过”的回归测试，覆盖前向兼容路径。
- 新增“高版本但缺少关键结构仍拒绝”的回归测试，验证错误信息会暴露缺失字段。
